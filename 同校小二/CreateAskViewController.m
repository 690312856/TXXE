...
@interface CreateAskViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,UIImagePickerControllerDelegate,CTAssetsPickerControllerDelegate>
@property (weak, nonatomic) IBOutlet UIButton *addImageButton;
@property (weak, nonatomic) IBOutlet UICollectionView *photoCollectionView;
@property (weak, nonatomic) IBOutlet LengthLimitedInputView *askTitleTextField;
@property (weak, nonatomic) IBOutlet LengthLimitedInputView *askDescriptionTextView;
@property (weak, nonatomic) IBOutlet LengthLimitedInputView *askPriceTextField;
@end

@implementation CreateAskViewController

- (CreateAskModel *)createAskModel
{
    if (!_createAskModel) {
        _createAskModel = [[CreateAskModel alloc]init];
    }
    return _createAskModel;
}
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    {
        self.title = @"编辑";
        for (NSString *urlText in self.createAskModel.netImageUrlTexts) {
    ...
    if (![NSString isNullOrNilOrEmpty:self.createAskModel.askTitle]) {
        self.askTitleTextField.text = self.createAskModel.askTitle;
    }
    if (![NSString isNullOrNilOrEmpty:self.createAskModel.askPrice]) {
        self.askPriceTextField.text = self.createAskModel.askPrice;
    }
    if (![NSString isNullOrNilOrEmpty:self.createAskModel.askDescription]) {
        self.askDescriptionTextView.text = self.createAskModel.askDescription;
    }
}
-(void)viewWillDisappear:(BOOL)animated
{
    [super viewWillDisappear:animated];
    self.createAskModel.askDescription = self.askDescriptionTextView.text;
    self.createAskModel.askPrice = self.askPriceTextField.text;
    self.createAskModel.askTitle = self.askTitleTextField.text;
}
...
{
    self.askDescriptionTextView.currentType = InputViewTypeTextView;
    self.askDescriptionTextView.maxLengthOfInputText = 200;
    self.askDescriptionTextView.font = [UIFont systemFontOfSize:13];
    self.askDescriptionTextView.placeholder = @"求购描述，不少于15个字";
    __weak __typeof(self)weakSelf = self;
    self.askDescriptionTextView.outOfLimitBlock = ^(id inputView){
        __strong __typeof(weakSelf)strongSelf = weakSelf;
        [strongSelf showTopMessage:[NSString stringWithFormat:@"求购描述不能超过%@个字！",@(strongSelf.askDescriptionTextView.maxLengthOfInputText)]];
    };
    
    self.askPriceTextField.currentType = InputViewTypeTextField;
    ((UITextField *)self.askPriceTextField.inputEareView).keyboardType = UIKeyboardTypeNumbersAndPunctuation;
    self.askPriceTextField.maxLengthOfInputText = 10;
    self.askPriceTextField.font = [UIFont systemFontOfSize:13];
    self.askPriceTextField.placeholder = @" 求购价格";
    self.askPriceTextField.outOfLimitBlock = ^(id inputView){
        __strong __typeof(weakSelf)strongSelf = weakSelf;
        [strongSelf.parentViewController showTopMessage:[NSString stringWithFormat:@"求购价格不能超过%@个字！",@(strongSelf.askPriceTextField.maxLengthOfInputText)]];
    };
    
    self.askTitleTextField.currentType = InputViewTypeTextField;
    self.askTitleTextField.maxLengthOfInputText = 30;
    self.askTitleTextField.font = [UIFont systemFontOfSize:13];
    self.askTitleTextField.placeholder = @" 求购名称";
    self.askTitleTextField.outOfLimitBlock = ^(id inputView){
        __strong __typeof(weakSelf)strongSelf = weakSelf;
        [strongSelf.parentViewController showTopMessage:[NSString stringWithFormat:@"求购名称不能超过%@个字！",@(strongSelf.askTitleTextField.maxLengthOfInputText)]];
    };
    
    for (UIView *view in @[self.askDescriptionTextView,self.askPriceTextField,self.askTitleTextField]) {
...
- (void)setSeletedImage:(UIImage *)image model:(ImageModel *)tempImageModel
{
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if ([tempImageModel saveImageOnDiskWithSize:CGSizeMake(600, 600)]) {
            [self.createAskModel.localImageModels addObject:tempImageModel];
            self.photoCollectionView.hidden = NO;
            self.addImageButton.hidden = NO;
            
            if (self.createAskModel.localImageModels.count == kMaxCountOfGoodImages) {
...
{
    return self.createAskModel.localImageModels.count;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ImageModel *imageModel = self.createAskModel.localImageModels[indexPath.item];
...
- (void)deleteWithImage:(ImageModel *)imageModel
{
    ...
    [SimplifyAlertView alertWithTitle:@"" message:@"是否确认删除？" operationResult:^(NSInteger selectedIndex) {
        ...
        }else if (selectedIndex == 1)
        {
            ...
            [self.createAskModel.localImageModels removeObject:imageModel];
            [self.photoCollectionView reloadData];
...
- (IBAction)nextStepBarBtnAction:(UIBarButtonItem *)sender {
    ...
    if ([self isValidToSubmit]) {
        NSDictionary *dict =[[NSDictionary alloc] initWithObjectsAndKeys:self.createAskModel,@"askModel",nil];
    //创建通知
        NSNotification *notification =[NSNotification notificationWithName:@"transAskModel" object:nil userInfo:dict];
    //通过通知中心发送通知
...
{
    BOOL should = YES;
    if (picker.selectedAssets.count >= (kMaxCountOfGoodImages - self.createAskModel.localImageModels.count)) {
        should = NO;
        UIAlertView *alertView =[[UIAlertView alloc] initWithTitle:nil
                                                           message:[NSString stringWithFormat:@"选择的图片不能超过%@张",@(kMaxCountOfGoodImages - self.createAskModel.localImageModels.count)]
                                                          delegate:nil
